...
import { Container, Grid, Typography, CardActionArea, Button, Box, IconButton, CardMedia, Card, Tooltip, CircularProgress, CssBaseline} from '@mui/material';
import { CustomCard, CustomCardContent, CustomBackground } from "../components/styled";
...
const RecipeList = ({ mealType, filters }) => {
  ...
  const toggleLike = (index) => {
    setLikedRecipes((prev) => ({
      ...prev,
      [index]: !prev[index], // Toggle the liked state for a specific recipe
    }));
  };
  ...
  const handleCopyClick = (url, index) => {
    ...
    // Show "Copied to clipboard" tooltip
    setCopiedTooltip((prev) => ({ ...prev, [index]: true }));
  
    // Force close the "Share" tooltip
    setShareTooltip((prev) => ({ ...prev, [index]: false }));
  
    setTimeout(() => {
      setCopiedTooltip((prev) => ({ ...prev, [index]: false })); // Hide copied tooltip
    }, 1500);
  };

  const handleInfoClick = (index) => {
    setInfoTooltip((prev) => ({ ...prev, [index]: !prev[index] }));
    setInformationTooltip((prev) => ({ ...prev, [index]: false })); // Hide "Information" tooltip
  };
  ...
  const fetchRecipes = (newFrom) => { 
    ...
    const API_URL = `https://api.edamam.com/search?q=${mealType}&app_id=${APP_ID}&app_key=${APP_KEY}&from=${newFrom}&to=${newFrom+RECIPES_PER_PAGE}${healthQuery}${dietQuery}`;

    fetch(API_URL)
      .then((res) => res.json())
      .then((data) => {
        if (data.hits) {
          setRecipes((prevRecipes) => [...prevRecipes, ...data.hits]); // Append new recipes
          setError("");
        } else {
  ...
  useEffect(() => {
    if (mealType) {
      setRecipes([]); // Reset when mealType changes
      setFrom(0);
      fetchRecipes(0);
    }
  }, [mealType, filters]);
    ...
    <Container maxWidth='md' sx={{ marginTop: '20px' }}>
      ...
      <Grid container spacing={3}>
        {recipes.map((item, index) => {
          const { label, image, source, totalTime, calories, url, yield:servings, dishType } = item.recipe;
          ...
          return (
            <Grid item key={index} xs={12} sm={6} md={4}>
              <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', borderRadius: 3, boxShadow: 3, minHeight: '400px', backgroundColor: theme => theme.palette.background.default   }}>
                ...
                <Box sx={{ display: 'flex'}}>
                  <Tooltip title={"I like this"} enterDelay={575} enterNextDelay={1000}>
